		...
		static unsafe object FetchJaggedArray (Type t, TFDataType dt, ref IntPtr data, long [] shape, int level = 0)
		{
			...
			if (level == shape.Length - 1) {
				...
				for (long l = 0; l < shape [level]; l++)
					switch (dt) {
					case TFDataType.Float:
						target.SetValue ((*(float*)data), (int) l);
						data += 4;
						break;
					case TFDataType.Double:
						target.SetValue ((*(double*)data), (int) l);
						data += 8;
						break;
					case TFDataType.Int32:
						target.SetValue ((*(int*)data), (int) l);
						data += 4;
						break;
					case TFDataType.UInt8:
						target.SetValue ((*(byte*)data), (int) l);
						data += 1;
						break;
					case TFDataType.Int16:
						target.SetValue ((*(short*)data), (int) l);
						data += 2;
						break;
					case TFDataType.Int8:
						target.SetValue ((*(sbyte*)data), (int) l);
						data += 1;
						break;
					case TFDataType.Int64:
						target.SetValue ((*(long*)data), (int) l);
						data += 8;
						break;
					case TFDataType.Bool:
						target.SetValue ((*(bool*)data), (int) l);
						data += 1;
						break;
					case TFDataType.Complex128:
						target.SetValue ((*(Complex*)data), (int) l);
						data += sizeof (Complex);
						break;
					case TFDataType.String:
				...
				if (top < Int32.MaxValue) {
					...
					for (long l = 0; l < top; l++) {
						...
						target.SetValue (chidArray, (int) l);
					}
				}
				return target;
			}
		...
		public object GetValue (bool jagged = false)
		{
			...
			if (dims == 1) {
				...
				if (jagged) {
					...
					var result = Array.CreateInstance (t, Shape.Select(Convert.ToInt32).ToArray());
